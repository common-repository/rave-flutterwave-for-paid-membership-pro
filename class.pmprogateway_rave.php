...
if (!function_exists('KKD_rave_pmp_gateway_load')) {
  ...
  static function pmpro_payment_option_fields($values, $gateway)
  {
    ?>
					...
					<tr class="gateway gateway_rave" <?php if ($gateway != "rave") { ?>style="display: none;"<?php 
						...
						<td>
							<select id="rave_payment_method" name="rave_payment_method">
								<option value="both" <?php if (empty($values['rave_payment_method'])) { ?>selected="selected"<?php 
                                                                                                  } ?>><?php _e('All', 'paid-memberships-pro'); ?></option>
								<option value="ussd" <?php if (empty($values['rave_payment_method'])) { ?>selected="selected"<?php 
                                                                                                  } ?>><?php _e('USSD only', 'paid-memberships-pro'); ?></option>
								<option value="account" <?php if (!empty($values['rave_payment_method'])) { ?>selected="selected"<?php 
                                                                                                      } ?>><?php _e('Account only', 'paid-memberships-pro'); ?></option>
								<option value="card" <?php if (!empty($values['rave_payment_method'])) { ?>selected="selected"<?php 
					...
					<tr class="gateway gateway_rave" <?php if ($gateway != "rave") { ?>style="display: none;"<?php 
						...
						<th scope="row" valign="top">	
							<label for="rave_public_key"><?php _e('Public Key', 'paid-memberships-pro'); ?>:</label>
						</th>
						<td>
							<input type="text" id="rave_public_key" name="rave_public_key" size="60" value="<?php echo esc_attr($values['rave_public_key']) ?>" />
							<small><?php _e("REQUIRED! Enter Rave Public key here", 'paid-memberships-pro'); ?></small>
						</td>
					</tr>
					<tr class="gateway gateway_rave" <?php if ($gateway != "rave") { ?>style="display: none;"<?php 
                                                                                            } ?>>
						<th scope="row" valign="top">	
							<label for="rave_secret_key"><?php _e('Secret Key', 'paid-memberships-pro'); ?>:</label>
						</th>
						<td>
							<input type="text" id="rave_secret_key" name="rave_secret_key" size="60" value="<?php echo esc_attr($values['rave_secret_key']) ?>" />
							<small><?php _e("REQUIRED! Enter Rave Secret key here", 'paid-memberships-pro'); ?></small>
						</td>
					</tr>
					<?php
  ...
  function kkd_pmpro_sendToRave(&$order)
  {
    ...
    $publicKey = pmpro_getOption("rave_public_key"); // Remember to change this to your live public keys when going live
    $secretKey = pmpro_getOption("rave_secret_key"); // Remember to change this to your live secret keys when going live
